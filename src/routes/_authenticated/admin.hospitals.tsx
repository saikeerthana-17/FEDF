...
import { CheckCircle2, XCircle, Building2, MapPin, Phone, Mail } from "lucide-react";
...
function AdminHospitalsPage() {
  ...
  const verify = async (id: string, val: boolean) => {
    ...
    toast.success(val ? "Hospital approved" : "Hospital revoked");
    load();
  };
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Approve hospital partner applications.</p>
        </div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 && (
          <Card><CardContent className="p-12 text-center text-sm text-muted-foreground">No hospitals in this view.</CardContent></Card>
        )}
        {filtered.map((h) => (
          <Card key={h.id}>
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-[10px] capitalize">{h.hospital_type?.replace("_", " ")}</Badge>
                </div>
                ...
                {h.address && <div className="mt-1 truncate text-xs text-muted-foreground">{h.address}</div>}
              </div>
              {h.is_verified ? (
